...
class TrainJobBaseline:
    def __init__(
        ...
        assert (
            not (eval_job_class is None) and do_eval
        ), "need to specify eval job class if eval is to be performed"
        ...
        self.test_dataloader = DataLoader(
            self.test_dataset, batch_size=self.batch_size, split="test"
        )

        if self.do_eval:
            self.test_trajectories = [
                self.test_dataset._get_trajectory() for _ in range(n_test_traj)
            ]
                        ...
                        if self.do_eval:
                            if seen % (self.batch_size * self.solving_interval) == 0:
                                for shuffles in self.search_shuffles:
                                    eval_job = self.eval_job_class(
                                        loggers=self.loggers,
                                        network=self.model.cpu(),
                                        shuffles=shuffles,
                                    )
