...
def _state_hash(state):
    if hasattr(state, "detach") and hasattr(state, "cpu") and hasattr(state, "numpy"):
        state = state.detach().cpu().numpy()

    if isinstance(state, np.ndarray):
        return np.ascontiguousarray(state).reshape(-1).tobytes()

    if isinstance(state, (list, tuple)):
        return tuple(_state_hash(item) for item in state)
...
class BestFSSolver:
    def __init__(
        ...
        self.value_estimator = self.value_estimator_class(
            network, checkpoint_path=checkpoint_path, metric=metric
        )
        self.buffer = []
    ...
    def solve(self, input, solved_state, problem_context=None):
        ...
        seen_hashed_states = {_state_hash(root.hash)}
        ...
        while True:
            ...
            if self.max_tree_depth == -1 or current_node.depth < self.max_tree_depth:
                goals, solving_subgoal = self.goal_builder.build_goals(
                    current_node.state
                )
                ...
                for child_num, goal_proposition in enumerate(goals):
                    ...
                    current_goal_state_hash = _state_hash(current_goal_state)
                    ...
                    if current_goal_state_hash not in seen_hashed_states:
                        ...
                        node_val = self.value_estimator.get_solved_distance(
                            new_node.state, solved_state
                        )

                        to_add_list.append(
                            (node_val, random.random(), goal_proposition)
                        )
                ...
                for child_num, (node_val, _, goal_proposition) in enumerate(
                    to_add_list
                ):
                    current_goal_state, current_path, _ = goal_proposition
                    current_goal_state_hash = _state_hash(current_goal_state)
        ...
        tree_metrics = {
            ...
            "avg_n_goals": (
                all_goals_created / expanded_nodes if expanded_nodes > 0 else 0
            ),
            "avg_dist_between_goals": (
                total_path_between_goals / all_goals_created
                if all_goals_created > 0
                else 0
            ),
        }
